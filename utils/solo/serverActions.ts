...
export async function fetchActiveSeason() {
  try {
    const { rows } = await pool.query(`
      ...
      LIMIT 1
    `);
    return rows.length > 0 ? rows[0] : { id: 6, season_number: 9, is_active: true };
  } catch (error) {
    console.error("Error fetching active season:", error);
    return { id: 6, season_number: 9, is_active: true };
  }
}
...
export async function createFinancialRule(rule: any) {
  try {
    ...
    `, [
      rule.name,
      rule.match_bonus_rc || 0, rule.match_bonus_rt || 0, rule.match_bonus_voucher || 0,
      rule.tournament_bonus_rc || 0, rule.tournament_bonus_rt || 0, rule.tournament_bonus_voucher || 0,
      rule.season_bonus_rc || 0, rule.season_bonus_rt || 0, rule.season_bonus_voucher || 0,
      rule.walkover_fine_rc || 0, rule.walkover_fine_rt || 0, rule.walkover_fine_voucher || 0,
      rule.match_extension_fee_rc || 0, rule.match_extension_fee_rt || 0, rule.match_extension_fee_voucher || 0
    ]);
    return rows[0];
  } catch (e) {
  ...
  try {
    const { rows } = await pool.query(`
      UPDATE career_financial_rules SET
        ...
        tournament_bonus_rc = $5, tournament_bonus_rt = $6, tournament_bonus_voucher = $7,
        season_bonus_rc = $8, season_bonus_rt = $9, season_bonus_voucher = $10,
        walkover_fine_rc = $11, walkover_fine_rt = $12, walkover_fine_voucher = $13,
        match_extension_fee_rc = $14, match_extension_fee_rt = $15, match_extension_fee_voucher = $16,
        updated_at = NOW()
      WHERE id = $17
      RETURNING *
    `, [
...
export async function updateFixture(id: number, homeScore: number | null, awayScore: number | null) {
  try {
    const { rows } = await pool.query(`
      UPDATE fixtures 
    ...
    console.error("Error updating fixture:", e);
    throw e;
  }
}
...
export async function nominateRwsCandidate(clubId: number, status: string) {
  try {
    const { rowCount } = await pool.query(`
      UPDATE tournament_teams 
      SET selection_status = $1 
      WHERE tournament_name = 'R2G World Series' AND club_id = $2
    `, [status, clubId]);
    
    if (rowCount === 0) {
      await pool.query(`
        INSERT INTO tournament_teams (tournament_name, club_id, selection_status)
        VALUES ('R2G World Series', $1, $2)
      `, [clubId, status]);
    }
    return { success: true };
  } catch (e) {
...
export async function applyTemplateAdjustment(
  ...
  try {
    ...
    } else if (type === 'season_bonus') {
      rc = Number(rule.season_bonus_rc) || 0;
      rt = Number(rule.season_bonus_rt) || 0;
      voucher = Number(rule.season_bonus_voucher) || 0;
...
export async function addClubToTournament(tournamentId: number, clubId: number) {
  try {
    const activeSeason = await fetchActiveSeason();
    const seasonId = activeSeason ? activeSeason.id : 6;
